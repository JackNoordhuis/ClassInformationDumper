...
/**
 * list-namespace-parts.php – ClassInformationDumper
 *
...
foreach($classes as $class) {
    foreach($parts = explode('\\', $class->getNamespace()) as $part) {
        if (! in_array($part, $namespaceParts)) {
            $namespaceParts[] = $part;
        }
    }
}
...
switch ($FORMAT) {
    case 'json':
        $lines[] = json_encode($namespaceParts);
        break;
    case 'php':
        $lines[] = '<?php'.PHP_EOL.PHP_EOL;
        $lines[] = Helper::stripBlankLines(Helper::stripNumericKeys(var_export($namespaceParts, true))).';';
        break;
    case 'serialize':
        $lines[] = serialize($namespaceParts);
        break;
}
