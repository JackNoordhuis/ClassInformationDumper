...
/**
 * list-traits-shortname.php – ClassInformationDumper
 *
...
$traits = (new \jacknoordhuis\classinformationdumper\DirectoryInformation($DIRECTORY))->getTraitModels();

$traitList = array_values(array_map(function (\jacknoordhuis\classinformationdumper\model\TraitModel $traitModel) {
    return $traitModel->getShortName();
}, $traits));

$lines = [];
switch ($FORMAT) {
    case 'json':
        $lines[] = json_encode($traitList);
        break;
    case 'php':
        $lines[] = '<?php'.PHP_EOL.PHP_EOL;
        $lines[] = Helper::stripBlankLines(Helper::stripNumericKeys(var_export($traitList, true))).';';
        break;
    case 'serialize':
        $lines[] = serialize($traitList);
        break;
}
