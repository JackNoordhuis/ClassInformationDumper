...
/**
 * list-constants.php – ClassInformationDumper
 *
...
foreach(array_merge($info->getClassModels(), $info->getInterfaceModels()) as $model) {
    foreach($model->getConstants() as $constant) {
        if(!$constant->isPrivate() and !in_array($constant->getName(), $constantList)) {
            $constantList[] = $constant->getName();
        }
    }
}
...
switch ($FORMAT) {
    case 'json':
        $lines[] = json_encode($constantList);
        break;
    case 'php':
        $lines[] = '<?php'.PHP_EOL.PHP_EOL;
        $lines[] = Helper::stripBlankLines(Helper::stripNumericKeys(var_export($constantList, true))).';';
        break;
    case 'serialize':
        $lines[] = serialize($constantList);
        break;
}
